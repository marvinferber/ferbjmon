...
public class JavaAgent implements ClassFileTransformer {
	...
	public static void premain(String agentArgument,
		...
		Runtime.getRuntime().addShutdownHook(
				new de.ubt.ferbjmon.callgraph.ShutdownHook());
	...
	private void enhanceMethod(CtBehavior method, String className)
		...
		for (CtClass parClass : method.getParameterTypes())
			if (!isNoTarget(parClass.getName()) && (!parClass.isPrimitive()))
				parbuilder
						.append("de.ubt.ferbjmon.callgraph.GraphCreator.pushParam(\""
								+ className
								+ "\",\""
								+ parClass.getName()
								+ "\");");
		...
		if (method instanceof CtMethod) {
			...
			if (!isNoTarget(realmethod.getReturnType().getName())
					&& (!realmethod.getReturnType().isPrimitive())
		...
		if (superc != null && !isNoTarget(superc.getName()))
			parbuilder
					.append("de.ubt.ferbjmon.callgraph.GraphCreator.pushParam(\""
							+ className + "\",\"" + superc.getName() + "\");");

		// Pushnode popnode einfügen
		method.insertBefore("de.ubt.ferbjmon.callgraph.GraphCreator.pushNode(\""
				+ className
				+ "\",\""
				+ method.getName()
				+ "\");"
	...
	private boolean isprimitiveArray(CtClass returnType) {
		try {
			if (returnType.isArray()
					&& (returnType.getComponentType().isPrimitive() || isNoTarget(returnType
